...
export const EditAnalysisDialog = ({ analysis, children }: EditAnalysisDialogProps) => {
  const [open, setOpen] = useState(false);
  const { clients, updateAnalysis } = useData();
  ...
  useEffect(() => {
    if (open) {
      ...
      // Use a small timeout to ensure the DOM is fully rendered before setting innerHTML
      const timer = setTimeout(() => {
        if (editorRef.current) {
          editorRef.current.innerHTML = analysis.notes || "";
        }
      }, 100);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.clientId || !notes) return;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {children || (
          <button className="p-2 rounded-full hover:bg-black/5 text-[#1d1d1f] transition-colors">
      ...
      <DialogContent className="sm:max-w-[850px] bg-[#f5f5f7] border-none p-0 overflow-hidden rounded-[28px] shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-0 flex flex-col h-[80vh]">
          ...
          <div className="flex-1 overflow-hidden flex flex-col bg-white">
            ...
            <div className="flex items-center gap-1 px-8 py-3 bg-[#f5f5f7]/60 border-b border-black/5">
              <div className="flex items-center gap-1.5 px-3">
                <button type="button" onClick={() => handleFormat('bold')} className="p-2 rounded-lg hover:bg-black/5 text-[#1d1d1f] transition-all"><Bold className="h-5 w-5" /></button>
                <button type="button" onClick={() => handleFormat('italic')} className="p-2 rounded-lg hover:bg-black/5 text-[#1d1d1f] transition-all"><Italic className="h-5 w-5" /></button>
                <button type="button" onClick={() => handleFormat('underline')} className="p-2 rounded-lg hover:bg-black/5 text-[#1d1d1f] transition-all"><Underline className="h-5 w-5" /></button>
              </div>
            </div>
          ...
          <div className="p-6 bg-[#f5f5f7]/80 backdrop-blur-md border-t border-black/5 flex justify-end gap-3">
            <Button type="button" onClick={() => setOpen(false)} className="h-12 px-6 rounded-2xl bg-black/5 text-[#1d1d1f] hover:bg-black/10 font-bold text-[15px]">Cancel</Button>
            <Button type="submit" className="h-12 px-10 rounded-2xl bg-[#95ec00] text-black hover:bg-[#a6ff00] font-extrabold text-[15px] shadow-sm transform transition-all active:scale-95">Update Analysis</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
